...
class _DayScreenState extends State<DayScreen> {
  ...
  Future<void> _openEditor(EntryType type, {Entry? existing}) async {
    final saved = await showEntryEditor(context, date: _key, type: type, existing: existing);
  ...
  Future<void> _addPhotos() async {
    ...
    try {
      ...
      if (mounted) messenger.showSnackBar(SnackBar(content: Text(t.photoAddFailed)));
    } finally {
    ...
    showModalBottomSheet(
      ...
      builder: (ctx) => SafeArea(
        child: Column(
          ...
          children: [
            _addTile(ctx, Icons.event_rounded, t.addSchedule, () => _openEditor(EntryType.schedule)),
            _addTile(ctx, Icons.check_circle_outline_rounded, t.addTodo, () => _openEditor(EntryType.todo)),
            _addTile(ctx, Icons.sticky_note_2_outlined, t.addMemo, () => _openEditor(EntryType.memo)),
            _addTile(ctx, Icons.add_photo_alternate_outlined, t.addPhoto, _addPhotos),
  ...
  Widget _addTile(BuildContext ctx, IconData icon, String label, VoidCallback onTap) {
    return ListTile(
      leading: Icon(icon),
      title: Text(label),
      onTap: () {
  ...
  Future<void> _confirmDelete(Entry e) async {
    ...
    final ok = await showDialog<bool>(
      ...
      builder: (ctx) => AlertDialog(
        ...
        content: Text(e.type == EntryType.photo ? t.deletePhotoConfirmBody : t.deleteConfirmBody),
        actions: [
  ...
  void _shiftDay(int delta) => setState(() => _date = DateTime(_date.year, _date.month, _date.day + delta));
  ...
  Widget build(BuildContext context) {
    ...
    return Scaffold(
      appBar: AppBar(
        title: Text(DateFormat(t.dateFormatFull, locale).format(_date)),
        actions: [
                ...
                if (all.isEmpty) {
                  return Center(
                    child: Padding(
                      ...
                      child: Text(t.emptyDay,
                          textAlign: TextAlign.center,
                          style: TextStyle(color: cs.onSurfaceVariant, height: 1.6)),
                    ),
                  );
                }
                final schedules = all.where((e) => e.type == EntryType.schedule).toList();
                ...
                return ListView(
                  ...
                  children: [
                    if (photos.isNotEmpty) ...[
                      _header(t.sectionPhoto, Icons.photo_outlined, '${photos.length}'),
                    ...
                    if (todos.isNotEmpty) ...[
                      _header(t.sectionTodo, Icons.check_circle_outline_rounded, null),
    ...
    return Padding(
      ...
      child: Row(
        children: [
          ...
          if (trailing != null) ...[
            const SizedBox(width: 6),
            Text(trailing, style: TextStyle(color: cs.onSurfaceVariant, fontSize: 12)),
          ],
        ],
      ),
    );
      ...
      itemBuilder: (context, i) {
        ...
        return GestureDetector(
          ...
          child: ClipRRect(
            ...
            child: Image.file(fileOf(e.thumb ?? e.file!), fit: BoxFit.cover,
  ...
  Widget _scheduleTile(Entry e, ColorScheme cs) {
    ...
    return Card(
      ...
      child: ListTile(
        ...
        title: Text(e.title),
        subtitle: e.body.isEmpty ? null : Text(e.body, maxLines: 2, overflow: TextOverflow.ellipsis),
    ...
    return Card(
      ...
      child: CheckboxListTile(
        ...
        title: Text(
          e.title,
          style: e.done
              ? TextStyle(decoration: TextDecoration.lineThrough, color: cs.onSurfaceVariant)
              : null,
        ),
    ...
    return Card(
      ...
      child: InkWell(
        ...
        child: Padding(
          ...
          child: Text(e.body, style: const TextStyle(height: 1.5)),
        ),
      ),
    );
  }
